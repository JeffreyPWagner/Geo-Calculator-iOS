...
class MainViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "segueToSetings" {
            if let destVC = segue.destination.children[0] as? settingsViewController {
    ...
    @IBAction func calculate(_ sender: UIButton) {
        ...
        distanceValue.text = String(d) + distanceUnits!;
    }
...
extension MainViewController: settingsViewControllerDelegate {
    func updateUnits(distanceUnits: String?, bearingUnits: String?) {
        ...
        distanceValue.text = bearingUnits;
        lat1.placeholder = distanceUnits;
        lat2.placeholder = bearingUnits;
    }
